...
class EfficientADLightning(pl.LightningModule):
    def __init__(
        self,
        backbones_type: str = "efficientad.MediumBackbones",
        in_channels: int = 6,
        ...
        backbones_params: dict | None = None,
    ):
        super().__init__()
        ...
        self.compute_percentile_quantiles = bool(
            pre.get("compute_percentile_quantiles", True)
        )
    ...
    def _maps(self, x: torch.Tensor) -> dict:
        ...
        if (t.shape[-2], t.shape[-1]) != (rh, rw):
            logging.getLogger(__name__).debug(
                f"Interpolating teacher feats from {(t.shape[-2], t.shape[-1])} to AE recon {(rh, rw)}"
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        if (
            self.use_imgNet_penalty
            and "imgNet_img" in batch
            and batch["imgNet_img"] is not None
        ):
    ...
    def _compute_teacher_mean_std(self, dl: DataLoader) -> None:
        ...
        disable = (
            False
            if getattr(self.trainer, "enable_progress_bar", True)
            else not (sys.stderr.isatty() or sys.stdout.isatty())
        )
        bar = tqdm(
            dl,
            total=total,
    ...
    def _compute_quantiles(self, dl: DataLoader) -> None:
        ...
        if len(maps_st) > 0:
            ...
            logging.getLogger(__name__).info(
                f"Quantiles computed: qa_st={self.qa_st.item():.6f} qb_st={self.qb_st.item():.6f} qa_ae={self.qa_ae.item():.6f} qb_ae={self.qb_ae.item():.6f}"
            )
        else:
            logging.getLogger(__name__).warning(
                "No good samples found in val loader; skipping quantiles (qa/qb remain defaults)"
            )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if "mask" in batch:
            ...
            pred = F.interpolate(
                pred, size=tgt.shape[-2:], mode="bilinear", align_corners=False
            ).squeeze(1)
    ...
    def on_train_epoch_end(self):
        try:
            ...
            if keys:
                ...
                logging.getLogger(__name__).info(
                    f"Train epoch {self.current_epoch} summary: {msg}"
                )
        except Exception as e:
            logging.getLogger(__name__).warning(
                f"Train epoch summary logging failed: {e}"
            )
    ...
    def on_validation_epoch_end(self):
        if len(self._val_preds) == 0:
            ...
            logging.getLogger(__name__).info(
                f"Validation epoch {self.current_epoch} summary: AUROC=0.0000, AP=0.0000 (no predictions/masks collected)"
            )
            return
        ...
        try:
            ...
            if reason:
                logging.getLogger(__name__).info(
                    f"Validation epoch {self.current_epoch} summary: AUROC={float(auroc):.4f}, AP={float(ap):.4f} ({reason})"
                )
            else:
                logging.getLogger(__name__).info(
                    f"Validation epoch {self.current_epoch} summary: AUROC={float(auroc):.4f}, AP={float(ap):.4f}"
                )
        except Exception as e:
            self.log("val/auroc", torch.tensor(0.0), prog_bar=True)
            self.log("val/ap", torch.tensor(0.0), prog_bar=False)
