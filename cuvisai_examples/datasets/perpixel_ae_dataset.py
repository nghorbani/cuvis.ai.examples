...
class PerPixelAECuvisDataSet(Dataset):
    ...
    def _load_cube(self, file_path: str, index: int) -> torch.Tensor:
        ...
        if self.normalize and self.mean is not None and self.std is not None:
            cube = torch.nn.functional.layer_norm(
                cube,
                cube.shape,
                torch.tensor(self.mean, dtype=cube.dtype, device=cube.device),
                torch.tensor(self.std, dtype=cube.dtype, device=cube.device),
            )
        if cube.shape[1] > self.max_img_shape or cube.shape[2] > self.max_img_shape:
            ...
            cube = torch.nn.functional.interpolate(
                cube.unsqueeze(0),
                size=(new_h, new_w),
                mode="bilinear",
                align_corners=False,
    ...
    def __getitem__(self, idx: int):
        ...
        if self._dbg < 3:
            logging.getLogger(__name__).debug(
                f"PerPixelAE item {idx}: cube shape={tuple(cube.shape)} channels_sel={self.channels}"
            )
            self._dbg += 1
        return {
            "image": cube,
            "label": torch.tensor(0, dtype=torch.long),
            "meta": {"path": file_path, "index": index},
        }
