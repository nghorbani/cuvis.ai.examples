...
def main(cfg: DictConfig):
    ...
    logging.info(f"Train dir={os.path.abspath(t_dir) if t_dir else 'N/A'} npz_files={t_npz if t_npz is not None else 'N/A'} size={t_n}")

    if val_ds is not None:
        v_dir, v_npz, v_n = _split_info(val_ds)
        logging.info(f"Val   dir={os.path.abspath(v_dir) if v_dir else 'N/A'} npz_files={v_npz if v_npz is not None else 'N/A'} size={v_n}")
        try:
            if t_dir and v_dir and os.path.abspath(t_dir) == os.path.abspath(v_dir):
                logging.warning("Train and Val directories are identical; splits may be overlapping and metrics biased.")
        except Exception:
            pass
    ...
    if len(train_ds) == 0:
        logging.warning("Train dataset is empty; training will no-op. Check dataset dirs and filters.")
