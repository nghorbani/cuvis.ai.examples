...
class EfficientADReporter:
    def __init__(
        self, out_dir: str, plot_thresholds: List[float] = None, overlay: str = "RGB"
    ):
    ...
    def save_images(
        self, batches: List[Dict[str, Any]], predictions: List[Dict[str, Any]]
    ):
        os.makedirs(self.out_dir, exist_ok=True)
        for i, (batch, pred) in enumerate(zip(batches, predictions)):
            img = (
                batch["image"].squeeze().detach().cpu().permute(1, 2, 0).numpy()
                if hasattr(batch["image"], "detach")
                else np.asarray(batch["image"])
            )
            ...
            fig, ax = plt.subplots(
                ...
                tight_layout=True,
                dpi=300,
                figsize=(8, 4 + len(self.plot_thresholds)),
            )
            rgb = (
                img[:, :, :3]
                if img.shape[2] >= 3
                else np.repeat(img[:, :, :1], 3, axis=2)
            )
            ...
            ax[0][1].set_title("IR")
            ax[1][0].imshow(
                (np.clip(a_map, 0, None) / (np.max(a_map) + 1e-8)) * 255,
                vmin=0,
                vmax=255,
            )
