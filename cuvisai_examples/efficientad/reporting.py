...
class Report:
    ...
    def generate_report(self):
        ...
        # for dataset_dir in self.config["dataset_dir"]:#self.config["labels"]):
        dataset_dir = Path(dataset_dir)
        cubes = glob.glob(str(dataset_dir / "*" / "*.npz"))
        # cubes = glob.glob(str(data_path / "*" / "*.cu3s"))
        cube_names = [Path(image).name for image in cubes]
        logger.info(f"Found {len(cubes)} cubes in {dataset_dir}")
        ...
        if self.create_images:
            for j, batch in enumerate(tqdm.tqdm(test_loader, desc="creating images")):
                rel_path = Path(cubes[j]).relative_to(dataset_dir)
