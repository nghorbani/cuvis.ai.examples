...
class Report:
    ...
    def create_inference_png(self, batch, pred, task: str):
        ...
        ax[int(len(rgb_images) / 2)][len(rgb_images) % 2].imshow(
            batch["mask"].squeeze().detach().cpu().numpy(),
            cmap=cmap,
            vmin=0,
    ...
    def per_class_anomaly_auroc(
        ...
        for label, label_name in enumerate(self.class_map):
            for batch_num, batch in enumerate(
                tqdm.tqdm(
                    data_loader,
                    desc=f"calculating per class AUROC for {label_name}",
    ...
    def per_class_dice(
        ...
        for label, label_name in enumerate(self.class_map):
            ...
            for batch_num, batch in enumerate(
                tqdm.tqdm(
                    data_loader,
                    desc=f"calculating per class Dice for {label_name}",
                    total=len(data_loader),
                )
            ):
                mask = torch.zeros_like(batch["mask"], device=self.device)
