...
from EfficientAD.data import EfficientADCuvisDataset
import yaml
import torch
import lightning as L
from EfficientAD.training import EfficientAD_lightning
...
class Report:
    ...
    def __init__(
        ...
        reporting_root_folder: pathlib.Path,
    ):
        self.config = config
        self.model = model
    ...
    def generate_report(self):
        ...
        for dataset_path, labels_path in zip(config["datasets"], config["labels"]):
            ...
            for p in pred:
                ...
                scores.append(score)
                all_scores.append(
                    p["anomaly_map"].squeeze(0).squeeze(0).detach().cpu().numpy()
                )
        ...
        if self.create_roc:
            fpr, tpr, roc_auc = self.plot_pixel_level_roc(
                np.array(all_scores), np.array(all_truths), normalize=False
            )
...
if __name__ == "__main__":
    ...
    model = EfficientAD_lightning.load_from_checkpoint(
        config["checkpoint_to_load"], config=config
    )
