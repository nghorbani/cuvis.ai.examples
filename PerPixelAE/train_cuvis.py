...
def train(config: dict) -> None:
    ...
    hsi_data = PerPixelAECuvisDataSet(
        os.path.realpath(config["datasets"]["train"]["root"]),
        mode="train",
        mean=config["mean"],
        std=config["std"],
    ...
    autoencoder_large_hybrid.save_params(
        f_params="./runs/final_ae.wts",
        f_optimizer="./runs/final_optimizer.pt",
        f_criterion="./runs/final_criterion.pt",
    )
